...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        P = my_alloc_page(p->p_pid);
        if (P == NULL
            || virtual_memory_map(p->p_pagetable, addr, (uintptr_t) P, PAGESIZE,
    ...
    // detect read-only program segments
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) 
    {   
        if (!(ph->p_flags & ELF_PFLAG_WRITE))
        {
            // map them as read-only for applications (PTE_P|PTE_U)
            virtual_memory_map(p->p_pagetable, addr, virtual_memory_lookup(p->p_pagetable, addr).pa, PAGESIZE,
